  ...
  let rec step (env : environment) (e : exp) : (environment * exp) =
    ...
    | Plus(e1, Num i) ->
      if e1 = True || e1 = False || e1 = TypeError then env, TypeError else
      let p = step env e1 in fst p, Plus(snd p, Num i)
    | Plus(Num i, e1) ->
      if e1 = True || e1 = False || e1 = TypeError then env, TypeError else
      let p = step env e1 in fst p, Plus(Num i, snd p)
    | Plus(e1, e2) ->
      if e1 = True || e1 = False || e2 = True || e2 = False || e1 = TypeError || e2 = TypeError then env, TypeError else
    ...
    | Apply(Lambda(var, body), Lambda(v, b)) -> if body = TypeError || b = TypeError then env, TypeError else ((var, Lambda(v, b))::env), body
    | Apply(Lambda(var, body), e2) -> if e2 = TypeError then env, TypeError else let p = step env e2 in ((var, e2)::env), Apply(Lambda(var, body), snd p)
    | Apply(e1, e2) ->if e1 = True || e1 = False || e1 = TypeError || e2 = TypeError then env, TypeError else let p = step env e1 in env, Apply(snd p, e2)
    | IsZero(False) -> env, TypeError
    | IsZero(e1) -> if e1 = TypeError then env, TypeError else let p = step env e1 in env, IsZero(snd p)
